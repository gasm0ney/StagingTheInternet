...
while(exit == 0 and (time.time() - start_time) < max_time):
    ...
    # If unison picked, have a random node conduct for a fixed amount of time.
    # Then, all return to phones once finished.
    # Thought: This is still human conducted although conductor randomly picked
    if(commands[0] == "unison"):
		print("All nodes in unison")
	    time.sleep(1)
	    for i in range(0, len(unison_text)):
			print(unison_text[i])
			time.sleep(0.5)
		for i in range(1, chorus_size+1):
    ...
    if(commands[0] == "monologue"):
    	...
    	for i in range(0, len(monologue)):
    		print(monologue[i])
    		time.sleep(0.5)
    	for i in range(1, chorus_size+1):
            chorus_states[i] = "phone"
